...
1. Add **RTK base-station setup** helper (`gnss-base` CLI, `pygpsclient.gnss_base`). Configures a u-blox (F9P / Here 4) as a survey-in or fixed base and enables the RTCM3 set (1005 + MSM4 + 1230); can persist to BBR+Flash. Ships a one-shot `rtcm-base-setup` systemd service that configures the base before the injector starts — see [docs/jetson_deploy.md](https://github.com/atifhalim/PyGPSClient/blob/master/docs/jetson_deploy.md).
1. Add headless **RTCM3 → MAVLink injector** (`rtcm-mavlink` CLI, `pygpsclient.rtcm_mavlink`). Reads RTCM3 from a serial base, a TCP caster or a file and injects it onto a MAVLink link as `GPS_RTCM_DATA` (fragmentation mirrors QGroundControl's `RTCMMavlink` exactly), e.g. onto a Herelink Wi-Fi stream so the autopilot relays corrections to the rover. Includes an RTK status monitor to validate delivery (rover fix type + `GPS_RTK` rate) — see [docs/rtcm_mavlink_validation.md](https://github.com/atifhalim/PyGPSClient/blob/master/docs/rtcm_mavlink_validation.md). Requires the optional `mavlink` extra (`pip install ".[mavlink]"`). Ships a `systemd` service and a Jetson Xavier NX deploy guide ([docs/jetson_deploy.md](https://github.com/atifhalim/PyGPSClient/blob/master/docs/jetson_deploy.md), `packaging/systemd/`, `packaging/udev/`) for running the injector always-on on a companion computer.
